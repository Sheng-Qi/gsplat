...
RenderMode = Literal["RGB", "d", "Ed", "D", "ED", "RGB-d", "RGB-Ed", "RGB+D", "RGB+ED", "RGB+F", "F", "RGB+DF", "RGB+EDF", "RGB+F-d", "RGB+F-Ed"]
...
def render_mode_has_color(mode: RenderMode) -> bool:
    return mode in {"RGB", "RGB-d", "RGB-Ed", "RGB+D", "RGB+ED", "RGB+F", "F", "RGB+DF", "RGB+EDF", "RGB+F-d", "RGB+F-Ed"}


def render_mode_has_hit_distance(mode: RenderMode) -> bool:
    return mode in {"d", "Ed", "RGB-d", "RGB-Ed", "RGB+F-d", "RGB+F-Ed"}


def render_mode_has_depth(mode: RenderMode) -> bool:
    return mode in {"D", "ED", "RGB+D", "RGB+ED", "RGB+DF", "RGB+EDF"}


def render_mode_has_flow(mode: RenderMode) -> bool:
    return mode in {"RGB+F", "F", "RGB+DF", "RGB+EDF", "RGB+F-d", "RGB+F-Ed"}


def render_mode_has_expected_depth(mode: RenderMode) -> bool:
    return mode in {"Ed", "ED", "RGB-Ed", "RGB+ED", "RGB+EDF", "RGB+F-Ed"}
...
def render_mode_has_only_depth_channel(mode: RenderMode) -> bool:
    return render_mode_has_depth_channel(mode) and not render_mode_has_color(mode) and not render_mode_has_flow(mode)


def render_mode_has_only_color(mode: RenderMode) -> bool:
    return not render_mode_has_depth_channel(mode) and render_mode_has_color(mode) and not render_mode_has_flow(mode)


def render_mode_has_only_flow_channel(mode: RenderMode) -> bool:
    return not render_mode_has_depth_channel(mode) and not render_mode_has_color(mode) and render_mode_has_flow(mode)
...
def rasterization(
    ...
    if render_mode_has_flow(render_mode):
    ...
    if with_ut:
        assert not render_mode_has_flow(render_mode), "Flow currently is not supported with with_ut."
    ...
    if render_mode_has_depth_channel(render_mode) and render_mode_has_color(
        ...
        if render_mode_has_hit_distance(render_mode):
            ...
            color_tensors.append(torch.zeros_like(depths[..., None]))
        else:
            # Gaussian depth modes: use projection depth
            color_tensors.append(depths[..., None])
            
        bg_tensors = [backgrounds] if backgrounds is not None else None
        if backgrounds is not None:
            bg_tensors.append(torch.zeros(batch_dims + (C, 1), device=backgrounds.device))

        if render_mode_has_flow(render_mode):
            assert (flows2d is not None), "Expected flows2d to be computed for flow mode."
            color_tensors.append(flows2d)
            if backgrounds is not None:
                bg_tensors.append(torch.zeros(batch_dims + (C, 2), device=backgrounds.device))
                
        colors = torch.cat(color_tensors, dim=-1)
        if backgrounds is not None:
            backgrounds = torch.cat(bg_tensors, dim=-1)
    ...
    elif render_mode_has_flow(render_mode) and render_mode_has_color(render_mode):
        assert (flows2d is not None), "Expected flows2d to be computed for flow mode."
    ...
    elif render_mode_has_only_flow_channel(render_mode):
