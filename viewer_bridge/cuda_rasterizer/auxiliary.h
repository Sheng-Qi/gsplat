...
__forceinline__ __device__ void colormap_ygb(float norm, float &r, float &g, float &b)
{
    float h = norm < 0.5f
        ? (240.0f + (120.0f - 240.0f) * (norm * 2.0f))  // 240→120
        : (120.0f + (60.0f  - 120.0f) * ((norm - 0.5f) * 2.0f)); // 120→60
    float s = 1.0f, v = 1.0f;
    float c = v * s;
...
__forceinline__ __device__ glm::vec3 hsv2rgb(const glm::vec3& c)
{
    float h = c.x * 6.0f;
...
void applyHeatmap(float value, float mod, float* rgbPtr)
{
    float norm = fminf(value / fmaxf(mod, 1e-6f), 1.0f);
    float r, g, b;
    colormap_ygb(norm, r, g, b);
    rgbPtr[0] = r;  rgbPtr[1] = g;  rgbPtr[2] = b;
}
