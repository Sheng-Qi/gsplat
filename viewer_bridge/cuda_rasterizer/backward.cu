	...
	bool antialiasing)
{
	...
	float3 mean4d = means[idx];
	float3 motion_i = motion[idx];
	float dt = timestamp - trbfcenter[idx];
	float3 mean = {
		mean4d.x + motion_i.x * dt,
		mean4d.y + motion_i.y * dt,
		mean4d.z + motion_i.z * dt
	};
	...
	if(antialiasing)
	{
		...
		const float d_h_convolution_scaling = dL_dopacity_v * opacity_time;
	...
	if (dL_dflow) {
		float2 dL_df = dL_dflow[idx];
		float3 p_view = t;
		float3 m_view = transformVec4x3(motion_i, view_matrix);
		float inv_z   = 1.0f / p_view.z;
		float inv_z2  = inv_z * inv_z;
		float inv_z3  = inv_z2 * inv_z;

		// loss w.r.t. m_view
		float dL_dmc_x = dL_df.x * ( h_x * inv_z );
		float dL_dmc_y = dL_df.y * ( h_y * inv_z );
		float dL_dmc_z = dL_df.x * ( -h_x * p_view.x * inv_z2 ) +
						dL_df.y * ( -h_y * p_view.y * inv_z2 );
		float3 dL_dmc = { dL_dmc_x, dL_dmc_y, dL_dmc_z };
		float3 dL_dmotion_i = transformVec4x3Transpose(dL_dmc, view_matrix);
		dL_dmotion[idx] = dL_dmotion_i;

		// loss w.r.t p_view (Gaussian mean t)
		float dflowx_dpx = -h_x * m_view.z * inv_z2;
		float dflowx_dpz = -h_x * m_view.x * inv_z2 + 2.f * h_x * p_view.x * m_view.z * inv_z3;
		float dflowy_dpy = -h_y * m_view.z * inv_z2;
		float dflowy_dpz = -h_y * m_view.y * inv_z2 + 2.f * h_y * p_view.y * m_view.z * inv_z3;
		float dL_dpview_x = dL_df.x * dflowx_dpx;
		float dL_dpview_y = dL_df.y * dflowy_dpy;
		float dL_dpview_z = dL_df.x * dflowx_dpz + dL_df.y * dflowy_dpz;
		dL_dtx += dL_dpview_x;
		dL_dty += dL_dpview_y;
		dL_dtz += dL_dpview_z;
	}
	...
	float* dL_dopacity)
{
	...
	// Account for motion
	glm::vec3 dL_dmotion_i = {
		dL_dmean.x * dt,
		dL_dmean.y * dt,
		dL_dmean.z * dt
	};
	dL_dmotion[idx] += dL_dmotion_i;
	float dL_dtc_1 = -(motion_i.x * dL_dmean.x
               + motion_i.y * dL_dmean.y
               + motion_i.z * dL_dmean.z);

	// Account for the time-based opacity
	float tscale = trbfscale[idx];
	float dt_scaled = dt / tscale;
	float gauss = expf(-dt_scaled * dt_scaled);
	float dL_dop = dL_dopacity[idx];
	dL_dopacity[idx] = dL_dop * gauss;
	float grad_scale = 2.f * opacities[idx] * dt * dt * gauss / (tscale * tscale * tscale);
	dL_dtrbfscale[idx] = dL_dop * grad_scale;
	float dL_dtc_2 = dL_dop * opacities[idx] * 2.f * dt * gauss / (tscale * tscale);
	
	float dL_dtc   = dL_dtc_1 + dL_dtc_2;
	dL_dtrbfcenter[idx] = dL_dtc;
}
...
renderCUDA(
	...
	const float* __restrict__ dL_doutflows,
	float3* __restrict__ dL_dmean2D,
	float4* __restrict__ dL_dconic2D,
	float* __restrict__ dL_dopacity,
	float* __restrict__ dL_dcolors,
	float* __restrict__ dL_dinvdepths,
	float2* __restrict__ dL_dflow
)
{
	...
	__shared__ float2 collected_flows[BLOCK_SIZE * 2];
	...
	if (inside)
	{
		...
		if (dL_doutflows)
			dL_dflow_pix.x = dL_doutflows[pix_id];
			dL_dflow_pix.y = dL_doutflows[H * W + pix_id];
	}

	float last_alpha = 0;
	float last_color[C] = { 0 };
	float last_invdepth = 0;
	float2 last_flow = { 0, 0 };
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			if (dL_doutflows)
			{
				collected_flows[block.thread_rank()] = flows[coll_id];
			}
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (dL_doutflows)
			{
				const float2 f = collected_flows[j];
				accum_flow_rec = {
					last_alpha * last_flow.x + (1.f - last_alpha)*accum_flow_rec.x,
					last_alpha * last_flow.y + (1.f - last_alpha)*accum_flow_rec.y
				};
				last_flow = f;
				dL_dalpha += (f.x - accum_flow_rec.x) * dL_dflow_pix.x
						+ (f.y - accum_flow_rec.y) * dL_dflow_pix.y;
				atomicAdd(&dL_dflow[global_id].x, dchannel_dcolor * dL_dflow_pix.x);
				atomicAdd(&dL_dflow[global_id].y, dchannel_dcolor * dL_dflow_pix.y);
			}
